...
{
    uint8_t *pYUVBuffer = new uint8_t[w*h*3/2];
    memcpy(pYUVBuffer, buffer, w * h);
    uint8_t *pU =buffer+w * h;
    uint8_t *pV =buffer+w*h*5/4;
    
    uint8_t *dy =pYUVBuffer+w * h;
...
-(UIImage *)YUVtoUIImage:(int)w h:(int)h buffer:(unsigned char *)buffer{
    ...
    /*unsigned char * yDest =  (unsigned char*)CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,0);
     unsigned char * uDest =  (unsigned char*)CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,1);
     unsigned char * vDest =  (unsigned char*)CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,2);
     memcpy(yDest, buffer, w * h);
     memcpy(uDest, buffer+w * h, w * h/ 4);
     memcpy(vDest, buffer+w*h*5/4, w * h/ 4);*/
