...
typedef struct tag_STRU_CAMERA_STATUS
{
    ...
    char appver[32];//界面固件版本号
    char oemid[32];//OEM 标示符
    char devname[96];//设备别名
    char devid[32];//设备UID
    int alarmstatus;
    int sdcardstatus;
    int sdcardtotalsize;
...
typedef struct tag_STRU_SD_RECORD_PLAN
{
    int record_plan1;
    int record_plan2;
    int record_plan3;
    ...
    int record_plan20;
    int record_plan21;
    int record_plan_enable;
    char mask[64];
}STRU_SD_RECORD_PLAN, *PSTRU_SD_RECORD_PLAN;

typedef struct tag_STRU_SET_SENSOR_STATUS
{
    int cmd;
    int nDoorbell;
    int nPtzspeed;
}STRU_SET_SENSOR_STATUS, *PSTRU_SET_SENSOR_STATUS;

typedef struct tag_STRU_SENSOR_STATUS{
    int nSensorStatus;
    int nAlarmStaus;
    int nSensorOpenCodeStatus;
    int nSensorDoorbellStatus;
}STRU_SENSOR_STATUS, *PSTRU_SENSOR_STATUS;


#define MAX_SENSOR_LIST_COUNT 32
typedef struct tag_STRU_SENSOR
{
    char nSensorName[64];
    int nSensorID;
    int nSensorid1;
    ...
    int nSensorStatus;
    int nPresetid;
}STRU_SENSOR, PSTRU_SENSOR;

typedef struct tag_STRU_SENSOR_LIST
{
    int nCount;
    STRU_SENSOR sensorList[MAX_SENSOR_LIST_COUNT];
}STRU_SENSOR_LIST, PSTRU_SENSOR_LIST;


typedef struct tag_STRU_SENSOT_GET_PRESET
{
    int cmd;/*0表示获取对应sensorid绑定的预置位， 1表示获取摄像机的看守位（此时sensorid无效）*/
    int sensorid;/*sensorid（当cmd=0时表示传感器的通道号（0-31），cmd=1时无意义）*/
    int presetid;/*presetid为绑定的预置位或者看守位*/
}STRU_SENSOR_GET_PRESET, *PSTRU_SENSOR_GET_PRESET;
