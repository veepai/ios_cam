...
public:
    ...
    int GetJsonCGI(const char* szDID, int cgi,char *json);
    ...
    int SetCameraSDCardStatusDelegate(char* szDID, id delegate);
    
    int SetSettingViewControllerParamsDelegate(char* szDID,id delegate);
    
    int SetCameraStatusDelegate(char* szDID, id delegate);
    int FirmwareUpgrade(const char* szDID, char* downloadserverAdd, char* downfilename, int FirmwareType);
