...
#import "CameraSDCardStatusProtocol.h"
#import "CameraStatusProtocol.h"
#import "PPPPAlarmProtocol.h"
#import "PPPPSensorListProtocol.h"
#import "PPPPSensorStatusProtocol.h"
#import "PPPPSensorAlarmProtocol.h"
#import "PPPPSensorAlarmLogProtocol.h"
#import "PPPPSensorGetPresetProtocol.h"
class CPPPPChannel
{
public:
    ...
    int Start();
    void SetStop();
    int cgi_livestream(int bstart, int streamid,int subStreamID);
    int cgi_get_common(char *cgi);
    int get_cgi(int cgi);
    int get_json_cgi(int cgi, char *buf);
    ...
    void SetCameraSDCardStatusDelegate(id delegate);
    
    void SetCameraStatusDelegate(id delegate);
    
    void SetSensorAlarmLogDelegate(id<PPPPSensorAlarmLogProtocol>delegate);
    
    void SetSensorListDelegate(id<PPPPSensorListProtocol>delegate);
    
    void SetSensorStatusDelegate(id<PPPPSensorStatusProtocol>delegate);
    
    void SetSensorAlarmDelegate(id<PPPPSensorAlarmProtocol>delegate);
    
    void SetSensorGetPresetDelegate(id<PPPPSensorGetPresetProtocol>delegate);
    
    void RefreshSnapshot(const char* szDID);
...
private:
    ...
    void ProcessRecordPlan(STRU_SD_RECORD_PLAN recordPlan);
    void ProcessMotionRecordPlan(STRU_SD_RECORD_PLAN recordPlan);
    void ProcessMotionPushPlan(STRU_SD_RECORD_PLAN recordPlan);
    void ProcessCameraStatusParams(STRU_CAMERA_STATUS cameraStatus);
    void ProcessPPPPAlarm(STRU_SENSOR_ALARM_INFO info);
    void ProcessSensorStatus(STRU_SENSOR_STATUS cmd);
...
private:
    ...
    NSCondition* m_PPPPAlarmLock;
    
    NSCondition *m_WifiParamsLock;
    
    NSRecursiveLock* m_SensorStatusLock;
    NSRecursiveLock* m_SensorAlarmLock;
    NSRecursiveLock* m_CameraSensorListDelegateLock;
    ...
    NSCondition* m_SDCardStatusLock;
    id<CameraSDCardStatusProtocol>m_SDCardStatusDelegate;
    
    NSCondition* m_CamStatusDelegateLock;
    id<CameraStatusProtocol>m_CamStatusDelegate;
    
    NSRecursiveLock* m_SensorGetPresetLock;
    id<PPPPSensorGetPresetProtocol> m_SensorGetPresetDelegate;
    
    NSRecursiveLock *m_SensorAlarmLogLock;
    id<PPPPSensorAlarmLogProtocol> m_SensorAlarmLogDelegate;
    
    NSRecursiveLock *m_SensorListLock;
    id<PPPPSensorListProtocol> m_SensorListDelegate;
    
    int m_bSensorAlarmThreadRuning;
