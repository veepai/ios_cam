...
+ (UIImage*) GetFirstImageFromRecordFile: (NSString*) strRecPath
{
    ...
    /*if (filehead.videoformat == 0) {//MJPEG
        ...
        if (datahead.dataformat == 0) { //必须是I帧
            ...
            int nWidth = 0;
            int nHeight = 0;
            int nRet = DecoderH264Frame((unsigned char*)pbuf, datahead.datalen, &yuv, &yuvlen, &nWidth, &nHeight);
            if (nRet > 0) {//decode success
                //yuv420->rgb565
                unsigned char *pRGB = new unsigned char[nWidth * nHeight * 2];
                unsigned char *y = yuv;
                unsigned char *u = yuv + nWidth * nHeight;
                unsigned char *v = yuv + nWidth * nHeight * 5 / 4;
                YUV420toRGB565((unsigned int*)pRGB, y, u, v, nWidth, nHeight);
