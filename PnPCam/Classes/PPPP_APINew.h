...
#define XQP2PC_API __declspec(dllexport)
#else
#define XQP2PC_API __declspec(dllimport)
#endif
#else
#include <time.h>
#include <sys/time.h>
...
//∑µªÿ∑˛ŒÒ∆˜ø…∑√Œ œ∏Ω⁄
typedef struct{
    struct sockaddr_storage svrAddr;//∑˛ŒÒ∆˜µÿ÷∑
    int	 ifResGet;// «∑Ò”–ªÿ”¶
...
extern "C" {
     ...
     */
    XQP2PC_API unsigned int XQP2P_GetAPIVersion();
    
    //◊˜∑œ
    XQP2PC_API int XQP2P_QueryDID(
                                  const char* DeviceName,
                                  char* DID,
                                  int DIDBufSize
                                  );
    
    XQP2PC_API int XQP2P_Initialize(
                                    char* server,
     ...
     */
    XQP2PC_API int XQP2P_InitializeExt(
                                       char* LicenseString,
                                       unsigned char ifIpv6Pri,
     ...
     */
    XQP2PC_API int XQP2P_DeInitialize( void );
    ...
    /*
     ...
     ERROR_PPPP_TIME_OUT
     */
    XQP2PC_API int XQP2P_NetworkDetect(
                                       st_PPPP_NetInfo1 *NetInfo,
                                       unsigned short UDP_Port
                                       );
    
    XQP2PC_API int XQP2P_NetworkDetectExt(
                                          st_PPPP_NetInfo1* NetInfo,
                                          UINT16 UDP_Port,
                                          int *svrNmb,
    ...
    /*
     ...
     ERROR_PPPP_NOT_INITIALIZED
     */
    XQP2PC_API int XQP2P_Share_Bandwidth(
                                         char bOnOff
                                         );
    ...
    XQP2PC_API int XQP2P_Listen_Break( void );
    
    XQP2PC_API int XQP2P_LoginStatus_Check( char* bLoginStatus );
    ...
    /*
     ...
     ERROR_PPPP_MAX_SESSION
     */
    XQP2PC_API int XQP2P_ConnectVgw( const char* TargetID, char bEnableLanSearch, unsigned short UDP_Port );
    ...
    /*
     ...
     ERROR_PPPP_MAX_SESSION
     */
    XQP2PC_API int XQP2P_ConnectByServer(
                                         const char *TargetID,
                                         char bEnableLanSearch,
                                         unsigned short UDP_Port,
                                         char *ServerString
    ...
    /*
     ...
     ERROR_PPPP_MAX_SESSION
     */
    XQP2PC_API int XQP2P_ConnectByIP(
                                     const char *TargetIP,
                                     const char *TargetID
                                     );
     ...
     */
    XQP2PC_API int XQP2P_Connect_Break();
     ...
     */
    XQP2PC_API int XQP2P_Check( int SessionHandle, st_PPPP_Session1* SInfo );
     ...
     */
    XQP2PC_API int XQP2P_Close(
                              int SessionHandle
                              );
    
    //same as the above now
    XQP2PC_API int XQP2P_ForceClose( int SessionHandle );
    ...
    /*
     ...
     ERROR_PPPP_SESSION_CLOSED_TIMEOUT
     */            
    XQP2PC_API int XQP2P_Check_Buffer( 
                                      int SessionHandle, 
                                      unsigned char Channel, 
    ...
    /*
     ...
     ERROR_PPPP_UDP_PORT_BIND_FAILED
     */
    XQP2PC_API int XQP2P_SearchExt(
    ...
    /*
     fun: break the session as the specified deviceID, maybe connecting or listening
     */
    XQP2PC_API int XQP2P_Break(char *szDID);
#ifdef __cplusplus
}
#endif // __cplusplus
